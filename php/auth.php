...
define('RATE_MAX_ATTEMPTS', (int)(getenv('RATE_LIMIT_ATTEMPTS') ?: 5));   // máximo de tentativas
define('RATE_WINDOW_MIN',   (int)(getenv('RATE_LIMIT_WINDOW')   ?: 15));  // janela em minutos
define('RATE_LOCKOUT_MIN',  (int)(getenv('RATE_LIMIT_LOCKOUT')  ?: 30));  // bloqueio em minutos
...
function checkRateLimit(string $ip): void {
    try {
        ...
        $lockout = date('Y-m-d H:i:s', time() - RATE_LOCKOUT_MIN * 60);

        // Conta tentativas FALHAS do IP dentro da janela de bloqueio
        $stmt = $pdo->prepare(
            'SELECT COUNT(*) as total FROM login_attempts
             WHERE ip_address = ? AND success = 0 AND attempted_at > ?'
        );
        $stmt->execute([$ip, $lockout]);
        $row = $stmt->fetch();
